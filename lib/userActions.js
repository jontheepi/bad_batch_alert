var UserActions = function() 
{
  ...
  self.userSetRegion = function(g, res, client, sender, action)
  {
    ...
    findQuery.on('row', function(row) {
      ...
      insertQuery.on('end', function() {
        ...
        var body;
        if (row.regions != null) {
          body = "👍 You have changed your region. You are now registered to get alerts in region " + region + ".\n\n" +
          "If you want to add more regions type 'add' followed by a number";
        } else {
          body = "👍 You are all set to receive alerts in region " + region + ".\n\n" +
  ...
  self.userVan = function (g,res,client,sender,action)
  {
    ...
    //EST daylight savings time rules. If we are between the 2nd Sunday in March and the first Saturday in November add an hour.
    var march = 2;
    var november = 10;
    var month = serverDate.getMonth();
    var date = serverDate.getDate();
    var day = serverDate.getDay();
    var year = serverDate.getYear() + 1900;
    var firstDay = new Date(year, month, 1).getDay();

    console.log("DST CALC VALS " + 'year:' + year + ', month:' + month + ', date:' + date + ', day:' + day, ', firstDay:' + firstDay); 

    //I'm sure there's a better way to figure this out.. sigh.
    var daysInWeek = 7;

    var dateOfFirstSat  = daysInWeek - firstDay;
    var dateOfSecondSun = (daysInWeek - firstDay)+1 + (firstDay != 0) ? daysInWeek:0;
    if (firstDay == 0) firstDay = daysInWeek;

    var isPast2ndSunInMarch = (month == march && date >= dateOfSecondSun);
    var isBetweenAprilAndOctober = (month > march && month < november);
    var isBeforeFirstSatInNovember = (month == november && date < dateOfFirstSat);

    if (isPast2ndSunInMarch || isBetweenAprilAndOctober  || isBeforeFirstSatInNovember ) {
      serverDate.setTime(serverDate.getTime() + (60*60*1000)); //add an hour.
